...
if [ -f /etc/grafana/grafana.ini ]; then
    ...
    if ! grep -q "^\[auth.anonymous\]" /etc/grafana/grafana.ini 2>/dev/null; then
        ...
        # enabled 설정 (auth.anonymous 섹션 내에서만)
        if grep -A 5 "^\[auth.anonymous\]" /etc/grafana/grafana.ini | grep -q "^enabled"; then
        ...
        # org_role 설정 (auth.anonymous 섹션 내에서만)
        if grep -A 5 "^\[auth.anonymous\]" /etc/grafana/grafana.ini | grep -q "^org_role"; then
            sed -i '/^\[auth\.anonymous\]/,/^\[/{s/^org_role.*/org_role = Viewer/}' /etc/grafana/grafana.ini
        else
            sed -i '/^\[auth\.anonymous\]/a org_role = Viewer' /etc/grafana/grafana.ini
        fi
    fi
